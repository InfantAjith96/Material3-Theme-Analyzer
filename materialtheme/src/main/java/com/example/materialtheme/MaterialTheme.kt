...
class MaterialTheme(context: Context) {
     ...
     */
    fun getTheme(): Int {
        return if (hasS() && isDynamicTheme) R.style.Theme_MaterialTheme_Dynamic else staticTheme.themeId
    }
    ...
    var staticTheme: Theme
        get() = Theme.entries[prefs.getInt(APP_THEME, 1)]
    ...
    companion object {
        private var INSTANCE: MaterialTheme? = null

        @JvmStatic
        fun getInstance(context: Context): MaterialTheme {
            ...
            synchronized(this) {
                val newInstance = MaterialTheme(context)
...
private class ThemeEngineActivityCallback : ActivityLifecycleCallbacks {
    override fun onActivityPreCreated(
        ...
        MaterialTheme.applyToActivity(activity)
    }
